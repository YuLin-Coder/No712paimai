(function($) {
	...
	function dataBackgroundImage() {
		$('[data-bgimg]').each(function() {
			...
			$(this).css({
				'background-image' : 'url(' + bgImgUrl + ')', // + meaning
																// concat
			});
		});
	}
	...
	$('.product_column5').on(
			...
			function(event) {
				$(event.target).find('.owl-item').removeClass('last').eq(
						event.item.index + event.page.size - 1)
						.addClass('last')
			}).owlCarousel(
			{
				autoplay : true,
	...
	$('.blog_thumb_active').owlCarousel(
			{
				...
				items : 1,
				margin : 20,
				navText : [ '<i class="ion-ios-arrow-thin-left"></i>',
	...
	$("#cat_toggle.has-sub > a").on(
			"click",
			function() {
				$(this).removeAttr('href');
				$(this).toggleClass('open').next('.categorie_sub').toggleClass(
						'open');
				$(this).parents().siblings().find('#cat_toggle.has-sub > a')
						.removeClass('open');
			});
				...
				function() {
					if ($(window).width() < 991) {
						...
						if (element.hasClass('open')) {
							...
							element.siblings('li').find('li').removeClass(
									'open');
	...
	$('.shop_toolbar_btn > button').on(
			...
			function(e) {
				...
				parentsDiv.removeClass('grid_3 grid_4 grid_5 grid_list')
						.addClass(viewMode);
				...
				if (viewMode == 'grid_list') {
					parentsDiv.children().addClass('col-12').removeClass(
							'col-lg-3 col-lg-4 col-md-4 col-sm-6 col-cust-5');
				}
	...
	/*---Off Canvas Menu---*/
	var $offcanvasNav = $('.offcanvas_main_menu'), $offcanvasNavSubMenu = $offcanvasNav
			.find('.sub-menu');
	$offcanvasNavSubMenu
			.parent()
			.prepend(
			...
			.on(
					...
					function(e) {
						...
						if (($this.parent().attr('class')
								...
								&& ($this.attr('href') === '#' || $this
										.hasClass('menu-expand'))) {
							e.preventDefault();
							if ($this.siblings('ul:visible').length) {
								$this.siblings('ul').slideUp('slow');
							} else {
								$this.closest('li').siblings('li').find(
